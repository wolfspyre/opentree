...
RUN apt-get -y update && DEBIAN_FRONTEND=noninteractive  apt-get install -y libhtml-template-perl apt-utils software-properties-common maven gnupg &&  apt-key adv --recv-keys --keyserver keyserver.ubuntu.com 0xF1656F24C74CD1D8 &&  add-apt-repository -y 'deb [arch=amd64,i386] http://sfo1.mirrors.digitalocean.com/mariadb/repo/10.2/debian sid main'
...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install mariadb-server-10.2 mariadb-server-core-10.2 galera-3
RUN mvn package
